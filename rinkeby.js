...
const web3 = new Web3(new Web3.providers.HttpProvider(process.env.ALCHEMY_ENDPOINT_RINKEBY));
let discourseHub = new web3.eth.Contract(DiscourseAbi, process.env.DISCOURSE_CONTRACT_ADDRESS_RINKEBY);
...
const setSchedule = async (body) => {
    return new Promise(async (resolve, reject) => {
        discourseHub.methods.scheduleDiscourse(+body.id, +body.timestamp).send({
            ...
            .then(result => {
                console.log("[Rinkeby]","Discourse scheduled at ", body.timestamp, " for proposal ", body.id);
                console.log(result);
                resolve(result);
            })
            .catch(err => {
                console.log("[Rinkeby]","Error Scheduling Discourse for", body.id, "at", body.timestamp);
                console.log(err);
                reject(err);
            })
    })
...
const setSpeaker = async (body) => {
    return new Promise(async (resolve, reject) => {
        discourseHub.methods.setSpeakerAddress(+body.id, body.handle, body.address).send({
            ...
            .then(result => {
                console.log("[Rinkeby]","Speaker set for ", body.id, "add:", body.address);
                console.log(result);
                resolve(result);
            })
            .catch(err => {
                console.log("[Rinkeby]","Error setting speaker for", body.id, "add:", body.address);
                console.log(err);
                reject(err);
            })
    })
...
const terminateProposal = async (id) => {
    return new Promise(async (resolve, reject) => {
        discourseHub.methods.terminateProposal(id).send({
            ...
            .then(result => {
                console.log("[Rinkeby]","Proposal terminated ", id);
                console.log(result);
                resolve(result);
            })
            .catch(err => {
                console.log("[Rinkeby]","Error terminating proposal", id);
                console.log(err);
                reject(err);
            })
    })
